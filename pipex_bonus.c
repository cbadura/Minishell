...
void	childfactory(int *fd, t_cmd *cmd, char **envp, t_envp *envl)
{
	...
	if (child_id == 0)
	{
		...
		execve(get_path(cmd->args[0], envp), cmd->args, NULL);
...
void	parentlabour(t_cmd_arr *cmd_arr, char **envp, t_envp *envl)
{
	...
	execve(get_path(cmd_arr->cmds[cmd_arr->size - 1]->cmd, envp), // leaks from getpath if cmd not found!!
		cmd_arr->cmds[cmd_arr->size - 1]->args, NULL);
